...
#         qwe[f"table1_{i}"] = table1_spans[i]
#     for i in range(1, 8, 2):
#         qwe[f"table0_{i}"] = table0_spans[i]
#     for i in range(1, 4, 2):
#         qwe[f"table2_{i}"] = table2_spans[i]
#     return qwe
    qwe = []
    for i in range(1, 8, 2):
...
print("Парсер поиска на росжкх\n")
url = advanced_search()
# В серче ссылка по которой переходят
parse = get_main_data(url)
